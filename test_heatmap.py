...
import src.web_interface.html_output as ho
...
filename_nested = list(utils.find_list_key(input_file, 'technical_replicates:filenames'))
...
for elem in ['project', 'experimental_setting', 'technical_details']:
    ...
    if elem == 'experimental_setting':
        input['experimental_setting'] = {'header': 'Experimental Setting',
                                 'plots': plot_list}
    else:
        input[elem] = {'header': header, 'html': ho.object_to_html(input_file[elem], 0, False)}
