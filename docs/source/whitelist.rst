...
The whitelists for the project are located in the Git repository `metadata_whitelists <https://github.com/loosolab/whitelists/>`_ inside the 'whitelists' folder. All newly created whitelist files must also be placed in this folder so that FRED can read them.
